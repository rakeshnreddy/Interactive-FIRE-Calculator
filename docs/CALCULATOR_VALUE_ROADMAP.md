...
| --- | --- | --- | --- | --- | --- |
| Compound Interest | Shows future value from starting amount, monthly contribution, term, and return. | Medium: formula is common, but monthly compounding is not always intuitive. | Add scenario comparison, contribution vs growth split, inflation-adjusted value, milestone dates, and annual contribution schedule. | Growth and Goal Studio | Stacked area for contributions vs growth, annual balance table, sensitivity grid by return and monthly contribution. |
| Savings Goal | Computes monthly savings needed for a target. | Medium: simple if user knows annuity math, but useful for planning. | Add deadline slider, catch-up options, confidence label, target inflation, savings schedule, and save-as-goal flow. | Growth and Goal Studio | Goal runway bar, deposit table, calendar milestone, monthly contribution scenarios. |
