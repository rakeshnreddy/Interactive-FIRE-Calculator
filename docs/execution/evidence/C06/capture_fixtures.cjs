 ...
 */
const { evaluateRenderedExpectation } = require('./rendered-expectations.cjs');
...
async function collectCase(browser, spec) {
  ...
  try {
    ...
    // Opaque shell/debug contrast only; does not prove child text over glass/gradients.
    const shellContrast = calculateContrastRatio(diag.themeInfo.textColor, diag.themeInfo.bgColor);
    const codeContrast = diag.codeTheme ? calculateContrastRatio(diag.codeTheme.color, diag.codeTheme.bgColor) : NaN;
