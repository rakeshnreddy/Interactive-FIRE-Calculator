...
- **Candidate Code Commit**: Pending fresh Astra commit after harness selector disambiguation (prior hosted candidate: `74671167fa6f373ccb8f979efad7efd1b188f66e`; base HEAD: `e5f4b7e`)<br>
- **Current Evidence HEAD**: `e5f4b7e`<br>
- **Branch / PR**: `codex/finpath-quality-execution` / [PR #140](https://github.com/rakeshnreddy/Interactive-FIRE-Calculator/pull/140)<br>
- **Immutable Preview URL**: Pending fresh preview deployment after Astra commit (prior candidate preview: https://11955f4b.interactive-fire-calculator.pages.dev)<br>
- **Deployment ID**: Pending fresh deployment (prior: `11955f4b-c177-4098-a102-13592a1e07da`)<br>
- **Effective Preview D1 Database**: `0dbad68e-7493-452f-8504-98d4c61ee5da` (`finpath-preview`)<br>
- **Candidate Status**: `BLOCKED` — The first hosted attempt on candidate commit `7467116` / preview `11955f4b` was **FAILED due to harness selector ambiguity** (`reviewPanelLocator.locator('.review-badge').boundingBox()` matched 3 badges: the header status badge plus `KEEP` and `DEFER` history badges, throwing a Playwright strict mode violation before Version 2 revise). The selector has now been scoped to `.panel-heading .review-badge` with `locateReviewPanelHeaderElements` requiring `count === 1` before measurement, and every later strict locator has been audited. Full local test suite passes (1,686 Vitest tests, 57 evaluator/collector unit tests with 45 negative tests, typecheck clean, production build clean). Both synthetic users from attempt `7467116` were independently verified fully cleaned up (0 rows across all 15 D1 tables, non-null tombstone, Clerk development user returned 404). Version 3 revise (B11-11) and B28 clearance (B28-13) remain **PENDING HOSTED PROOF** pending Astra git commit, exact-code CI, isolated preview deployment, and live hosted proof. Checkpoint C10 remains locked.<br>
...
| **B28-13** | Review panel sticky topbar clearance | **PENDING HOSTED PROOF** | Product implementation complete: `.planning-review-panel` layout specifies `scroll-margin-top: calc(var(--topbar-height, 72px) + 1.5rem)` (mobile: `calc(var(--topbar-height, 72px) + 1.25rem)`); `PlanningWorkspace.tsx` honors `prefers-reduced-motion: reduce`. Runner harness updated to use `switchTheme` across 4 viewport/theme cases, verify computed theme tokens (`surface`, `heading`), navigate by keyboard (Tab) to a real interactive review control in `.planning-review-panel`, assert `document.activeElement`, and assert strictly positive clearance from sticky topbar for focused control, heading, and badge via scoped `locateReviewPanelHeaderElements` (requiring `count === 1` for header heading and status badge). Hosted attempt `7467116` was FAILED due to harness selector ambiguity (`.review-badge` matched 3 badges after keep/defer reviews); cleanup verified. Collector positive test demonstrates corrected selector isolates header badge and obtains bounding box amidst multiple history badges; collector negative tests 36, 41, and 42 verify fail-closed behavior on occlusion or duplicate/missing header badges. Local layout verification via synthetic diagnostic fixture (`scripts/measure_local_clearance.mjs`) confirmed strictly positive clearance (>86px) and 0 horizontal overflow across all 4 cases. Live clearance and focus/keyboard proof on the real authenticated product remain PENDING HOSTED PROOF on a fresh isolated preview deployment. |
