...
- **Candidate Code Commit**: Pending Astra commit of working tree product changes (base HEAD: `ad2842e`; previous accepted B35 candidate: `5dda3d2be24246e3470a65e7653a0b6e425cbece`)<br>
- **Current Evidence HEAD**: `ad2842e`<br>
- **Branch / PR**: `codex/finpath-quality-execution` / [PR #140](https://github.com/rakeshnreddy/Interactive-FIRE-Calculator/pull/140)<br>
- **Immutable Preview URL**: Pending fresh preview deployment after Astra commit (prior preview: https://51acbf88.interactive-fire-calculator.pages.dev)<br>
- **Deployment ID**: Pending fresh deployment (prior: `51acbf88-db0a-47d1-b399-814dad835a9b`)<br>
- **Effective Preview D1 Database**: `0dbad68e-7493-452f-8504-98d4c61ee5da` (`finpath-preview`)<br>
- **Candidate Status**: `BLOCKED` — product changes and hosted-runner readiness corrections implemented and verified locally for B11 (truthful synthetic seed via `calculateFirePlan`, financial revision: annualExpense 50k -> 45k, truthful banner, calculator context bar with return route, server-derived replay key, real 3-review history verification with scoped revise row count and preserved total count of 3, strict deployment inputs validation `C09_CANDIDATE_SHA`/`C09_DEPLOYMENT_ID`/`C09_PREVIEW_URL`, byte-exact V1/V2 immutability across versions/inputs/results with raw D1 schema parsing) and B28 (reduced-motion scroll support, scroll-margin-top expanded to `calc(var(--topbar-height, 72px) + 1.5rem)`, real theme switching via `switchTheme` with computed token assertions, keyboard Tab navigation to active review control with positive topbar clearance for control, heading, and badge across 4 cases, and fixture diagnostic cases under `local-fixture/`). Full local test suite passes (1,686 Vitest tests, 55 evaluator/collector unit tests, typecheck clean, production build clean). B11-11 and B28-13 marked PENDING HOSTED PROOF pending Astra git commit, exact-code CI, isolated preview deployment, and live hosted proof.<br>
...
| **B28-13** | Review panel sticky topbar clearance | **PENDING HOSTED PROOF** | Product implementation complete: `.planning-review-panel` layout specifies `scroll-margin-top: calc(var(--topbar-height, 72px) + 1.5rem)` (mobile: `calc(var(--topbar-height, 72px) + 1.25rem)`); `PlanningWorkspace.tsx` honors `prefers-reduced-motion: reduce`. Runner harness updated to use `switchTheme` across 4 viewport/theme cases, verify computed theme tokens (`surface`, `heading`), navigate by keyboard (Tab) to a real interactive review control in `.planning-review-panel`, assert `document.activeElement`, and assert strictly positive clearance from sticky topbar for focused control, heading, and badge. Collector negative test 41 confirms failure if focused control is occluded by topbar. Local layout verification via synthetic diagnostic fixture (`scripts/measure_local_clearance.mjs`) confirmed strictly positive clearance (>86px) and 0 horizontal overflow across all 4 cases: (1) Desktop 1280px light: clearance = 98.4px (badge 86.4px); (2) Desktop 1280px dark: clearance = 98.4px (badge 86.4px); (3) Mobile 320px light: clearance = 88.8px (badge 127.0px); (4) Mobile 320px dark: clearance = 88.8px (badge 127.0px). Verified screenshots saved strictly to `docs/execution/evidence/C09/local-fixture/`: `local_fixture_desktop_light.png`, `local_fixture_desktop_dark.png`, `local_fixture_mobile_light.png`, `local_fixture_mobile_dark.png`. Tracked hosted screenshot `04_b11_review_completed_panel_focused.png` remains preserved from committed HEAD. Live clearance and focus/keyboard proof on the real authenticated product remain PENDING HOSTED PROOF on a fresh isolated preview deployment. |
