...
| C07-06b | CSV import | BLOCKED | Browser file chooser opened, but automation returned exact error `fileChooser.setFiles failed: {"code":-32000,"message":"Not allowed"}`. No approval rationale was returned; the import was not claimed as tested. |
| C07-06c | Cross-user reads and writes rejected | PARTIAL / BLOCKED | B's authenticated account view was empty while A's row existed, proving read isolation. A direct authenticated B-to-A write with observable HTTP response was not completed before identity cleanup, so write isolation remains unproven. |
| C07-07 | Delete data, late-write 410, provider cleanup | PARTIAL / BLOCKED | A and B each used the signed-in deletion UI successfully. A stayed signed in and an immediate profile save produced `Profile could not be saved`; the browser did not expose HTTP status, so this is not claimed as a verified 410. Clerk showed `No users yet` after deleting exactly A/B. D1 retained exactly two deleted-user tombstones and returned zero rows for those IDs across all 14 user-scoped tables. |
...
- Clerk development user grid: `No users yet` after deleting only the exact synthetic A/B rows.
- D1 `users`: A `user_3JOGiP2nXKPm7UiZTBk27WNy2SF` and B `user_3JOH4WytkmA7wx1Xq7BoaRl3sW5` remain as tombstones with non-null `deleted_at`.
- D1 row counts scoped to those IDs were zero in `user_profiles`, `financial_accounts`, `account_balances`, `transactions`, `goals`, `plans`, `plan_versions`, `fire_plan_inputs`, `fire_plan_results`, `assumptions`, `balance_imports`, `transaction_imports`, `saved_calculator_results`, and `audit_log`.
