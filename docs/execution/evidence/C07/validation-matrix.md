...
| C07-06b | CSV import | BLOCKED | Direct element `setInputFiles` harness implemented in [`run_remaining_proofs.mjs`](remaining/run_remaining_proofs.mjs) (R1–R4 repaired; verified with 11/11 tests in [`run_remaining_proofs.test.mjs`](remaining/run_remaining_proofs.test.mjs)). Blocked before execution at Step 2 due to missing `CLERK_SECRET_KEY` in `.env.preview.local` needed for Clerk testing-token Turnstile bypass. See [`remaining/report.json`](remaining/report.json). |
| C07-06c | Cross-user reads and writes rejected | PARTIAL / BLOCKED | Read isolation verified in earlier run (B saw 0 accounts). Direct authenticated B-to-A write rejection harness implemented in [`run_remaining_proofs.mjs`](remaining/run_remaining_proofs.mjs) (R1–R4 repaired; verified with [`run_remaining_proofs.test.mjs`](remaining/run_remaining_proofs.test.mjs)). Blocked before execution at Step 2 pending `CLERK_SECRET_KEY`. See [`remaining/report.json`](remaining/report.json). |
| C07-07 | Delete data, late-write 410, provider cleanup | PARTIAL / BLOCKED | Data deletion verified in earlier run. HTTP 410 assertion on immediate post-deletion `PUT /api/profile` and fail-closed cleanup implemented in [`run_remaining_proofs.mjs`](remaining/run_remaining_proofs.mjs) (R1–R4 repaired; verified with [`run_remaining_proofs.test.mjs`](remaining/run_remaining_proofs.test.mjs)). Blocked before execution at Step 2 pending `CLERK_SECRET_KEY`. Historical cleanup verified: 2 tombstones preserved in D1 `users`, 0 scoped rows across 14 tables ([`remaining/cleanup.json`](remaining/cleanup.json)). |
