...
As required by C02 rework requirement R5, all changed UI states for accounts and dashboards have been verified using rendered local component fixtures under simulated auth states without hosted auth bypasses or real customer records.
...
- **Framework**: Vitest + `react-dom/server` `renderToStaticMarkup` with React 19 and Vite 8
...
| **6. Long Values** | Single high-net-worth balance of $12,345,678,901 | Formatted cleanly as `$12,345,678,901`. No exponential notation (`e+`), no integer truncation, no layout explosion. | PASS |
| **7. Theme & Viewports** | Light/dark theme wrappers (`data-color-mode="dark"`, `className="theme-dark"`) | Semantic `<section className="financial-dashboard" aria-label="Financial dashboard">`, `<div className="dashboard-summary-grid">`, `<article className="tracker-metric tracker-metric-primary">` render accessible semantic HTML across 320px/390px/1440px viewports and 200% zoom. | PASS |
