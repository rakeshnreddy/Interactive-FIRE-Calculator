...
export const onRequestPost: PagesFunction<BalanceImportCommitEnv> = async ({ request, env }) => {
  ...
  try {
    return json(await commitBalanceImport(db.database, session.auth.userId, parsed.value), 201);
  } catch (error) {
    if (error instanceof DuplicateBalanceImportError) {
